...
# 04 August 2021
# License MIT
# Transport Phenomena: Python Quizz Point 1
...
class Optimice:
    def objective_Colebrook(self, x, *args):
        ...
        return (1 / np.sqrt(x1)) + (2.0 * np.log10(((eps / (np.sqrt(4 * 0.01/np.pi*x2))) / 3.7) + (2.51 / (
                    (rho * (np.sqrt(4 * 0.01 / np.pi * x2) * x2) / miu) * np.sqrt(x1)))))
    ...
    def constraint_hl_eq_f(self, x, *args):
        ...
        return x3 - (x1 * (L * (x2 ** 2) / (np.sqrt(4 * 0.01/np.pi*x2) * 2 * g)))
...
sol = minimize(Opt.objective_Colebrook, x0, method='SLSQP', constraints=constraint, args=(eps, rho, miu, L, g, z_4, z_1),
