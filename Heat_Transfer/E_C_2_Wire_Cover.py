...
class PVC_1_cover:
    ...
    def B_C_2_r_max_Resis(self,E_gen,Radius_min_pvc_1,Radius_max_pvc_1,T_surface_in_pvc_1,K_conductiviy_pvc,C1_pvc_1,length_wire):
        E = E_gen  # [W/m^3]
...
class Optimice_Copper_Alone:
    ...
    def constraint_equal_Q_Resist(self,x):
        ...
        x3 = x[2]  # C2 [K]
        x4 = x[3]  # Q [W]
        return (x4*((1/2.5*np.pi*2.32*25)*(1/0.74*5.670e-8*((-x1*((2.32/2)**2)/4*386)+x2*np.log((2.32/2))+ x3-217+273.15)
                                          *(((-x1*((2.32/2)**2)/4*386)+x2*np.log((2.32/2))+ x3)**2-(217+273.15)**2)))
                /((1/2.5*np.pi*2.32*25)+(1/0.74*5.670e-8*((-x1*((2.32/2)**2)/4*386)+x2*np.log((2.32/2))+ x3-217+273.15))*
               (((-x1*((2.32/2)**2)/4*386)+x2*np.log((2.32/2))+ x3)**2-(217+273.15)**2)))-\
...
Q_gen = Cop_Wir.BC2_Q_eq_Therm_Cont_Resist(55000,((2.32/1000)/2)*25*2*np.pi,T_gap+273.15,75+273.15) #[W]--Power
...
C2_pvc_1 = PVC_1.B_C_2_r_max_Resis(E_gen,((2.32/1000)/2),(((2.32/2)+0.76)/1000),75+273.15,0.17,C1_pvc_1,25)
...
#Solve for PVC2
PVC_2 = PVC_2_cover()
C1_pvc_2 = PVC_2.B_C_1_Q_conv_eq_cond(25,0.92,Prof_pvc_1[759],T_surr_in[1],(((2.32/2)+0.76)/1000),7.285/1000,0.17)
...
x0 = [ 1.28212852e-02,  7.04142154e+02,  3.87306067e+02, -1.35499415e+00] # This inital values are extracted from a first solution given by the method
